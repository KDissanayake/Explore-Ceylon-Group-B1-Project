...
class ChangePasswordForm extends StatefulWidget {
  final User? userProfile; // Add userProfile property here
  const ChangePasswordForm({Key? key, required this.userProfile})
      : super(key: key);
  @override
...
class _ChangePasswordFormState extends State<ChangePasswordForm> {
  ...
  @override
  Widget build(BuildContext context) {
    final bool isGoogleSignIn = widget.userProfile!.providerData
              ...
              child: Form(
                ...
                child: Column(
                  children: [
                    ...
                    if (!isGoogleSignIn) // Show only for non-Google sign-in users
                      SizedBox(height: 20),
                    ElevatedButton(
...
class _PreferencesFormState extends State<PreferencesForm> {
  ...
  late Map<String, String> _preferenceColors = {};
    ...
    if (preferencesSnapshot.docs.isNotEmpty) {
      setState(() {
        _preferences = preferencesSnapshot.docs
            ...
            .toList()
            .cast<String>();
        _preferenceColors = Map.fromIterable(
          preferencesSnapshot.docs,
          key: (doc) => doc.get('name'),
          value: (doc) => doc.get('color') ?? '#FFFFFF',
        );
      });
  ...
  void _fetchUserPreferences() async {
    ...
    if (preferences.exists) {
      ...
      if (userPreferences != null) {
        setState(() {
          _selectedPreferences = userPreferences
  ...
  void _togglePreference(String preference) {
    ...
    _updateUserPreferences(); // Call to update user preferences
  }

  void _updateUserPreferences() {
    // Update user preferences in Firestore
    List<Map<String, dynamic>> preferencesWithColor = [];
    _selectedPreferences.forEach((preference) {
      preferencesWithColor.add({
        'preference': preference,
        'color': _preferenceColors[preference] ?? '#FFFFFF',
      });
    });
    ...
    FirebaseFirestore.instance
        .collection('userPreferences')
        .doc(widget.userProfile!.uid)
        .set({'preferences': preferencesWithColor});
  }

  @override
  Widget build(BuildContext context) {
    // Fetch user preferences in the build method
    _fetchUserPreferences();
      ...
      children: _preferences.map((preference) {
        return ChoiceChip(
          ...
          selectedColor: Color(int.parse(
              _preferenceColors[preference]!.replaceAll('#', '0xFF'))),
          ...
          elevation: _selectedPreferences.contains(preference) ? 2 : 0,
          shadowColor: _selectedPreferences.contains(preference)
              ? Colors.black
              : Colors.transparent,
        );
      }).toList(),
